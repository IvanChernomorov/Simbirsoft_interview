...
+ `Iterator<E> iterator()`: возвращает объект `Iterator`. Из интерфейса `Iterable`    
...
| Доступ к элементам | По индексу            | Через [итератор]                | По ключу                                    |
...
`TreeSet` основан на бинарном дереве поиска.  Его особенность заключается в том, что элементы множества сортируются. Для простых типов это стандартная сортировка (для чисел - по возрастанию, для - строк в лексико-графическом порядке и т.д.). Для сравнения объектов используются [компараторы]().
